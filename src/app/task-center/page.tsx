...
export default function TaskCenterPage() {
  ...
  if (!mounted || isLoading) {
    return (
      <div className="flex h-svh items-center justify-center bg-primary">
        ...
        <section className="bg-white/40 backdrop-blur-xl rounded-[3rem] p-8 border border-white/40 relative overflow-hidden shadow-2xl">
          ...
          <div className="grid grid-cols-4 gap-3 mb-4">
            {REWARDS.slice(0, 4).map((reward, i) => {
              ...
              return (
                <div key={i} className={cn("aspect-square rounded-[1.75rem] border flex flex-col items-center justify-center gap-2 transition-all duration-500", isActive ? "bg-primary border-primary shadow-lg" : isCurrent ? "bg-white/60 border-primary animate-pulse" : "bg-white/20 border-white/30")}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {REWARDS.slice(4).map((reward, i) => {
              ...
              const isCurrent = (streak % 7) + 1 === dayNum && canClaim;
              return (
                <div key={i} className={cn("aspect-square rounded-[2rem] border flex flex-col items-center justify-center gap-2 transition-all relative", isActive ? "bg-primary border-primary shadow-lg" : isCurrent ? "bg-white/60 border-primary animate-pulse" : "bg-white/20 border-white/30")}>
