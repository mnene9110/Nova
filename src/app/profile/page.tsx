...
export default function ProfilePage() {
  ...
  return (
    <div className="flex flex-col h-svh w-full bg-gradient-to-b from-[#EB4C4C] via-white/50 to-white text-gray-900 overflow-y-auto scroll-smooth">
      <header className="flex flex-col items-center pt-12 pb-6 px-6 shrink-0 relative">
        <div className="relative mb-6 group">
          <div className="absolute -inset-2 bg-gradient-to-tr from-primary to-orange-400 rounded-[3rem] blur-xl opacity-20 group-hover:opacity-40 transition duration-1000"></div>
          ...
          <button 
            ...
            className="absolute bottom-1 right-1 w-11 h-11 rounded-2xl bg-zinc-900 flex items-center justify-center shadow-xl active:scale-90 transition-all border-none outline-none ring-0 select-none"
          >
        ...
        <div className="flex flex-col items-center gap-2 mb-4">
          ...
          <button 
            onClick={copyId} 
            className="flex items-center gap-2 px-5 py-2 glass-card rounded-full active:scale-95 transition-all native-shadow border-white/30 outline-none select-none ring-0"
          >
        ...
        <div className="flex gap-2 flex-wrap justify-center mt-2">
          {userProfile?.isAdmin && (
            <span className="px-3 py-1 bg-white/20 text-white border border-white/30 rounded-full text-[8px] font-black uppercase tracking-widest">Administrator</span>
          )}
          {userProfile?.isSupport && (
            <span className="px-3 py-1 bg-white/20 text-white border border-white/30 rounded-full text-[8px] font-black uppercase tracking-widest">Official Support</span>
          )}
          {userProfile?.isCoinseller && (
            <span className="px-3 py-1 bg-amber-500/20 text-white border border-amber-500/30 rounded-full text-[8px] font-black uppercase tracking-widest">Official Coinseller</span>
          )}
        </div>
      </header>
      ...
      <main className="flex-1 px-6 space-y-10 pb-44">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="glass-card rounded-[2.5rem] p-6 flex flex-col items-center gap-3 native-shadow border-white/60">
            ...
            <Button 
              ...
              className="w-full h-9 rounded-2xl bg-primary hover:bg-primary/90 text-white font-black uppercase text-[9px] tracking-widest native-shadow select-none outline-none border-none ring-0"
            >
              Recharge
            </Button>
          </div>
          ...
          <div className="bg-zinc-900 rounded-[2.5rem] p-6 flex flex-col items-center gap-3 native-shadow border border-zinc-800">
            ...
            <Button 
              ...
              className="w-full h-9 rounded-2xl bg-white/10 hover:bg-white/20 text-white border border-white/10 font-black uppercase text-[9px] tracking-widest select-none outline-none ring-0"
            >
              Income
            </Button>
          </div>
        ...
        {hasAdminPrivileges && (
          <div className="space-y-4">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-primary ml-4">Administrative Dashboards</h3>
            <div className="grid grid-cols-1 gap-3">
              {userProfile?.isAdmin && (
                <button 
                  onClick={() => router.push('/admin/roles')} 
                  className="w-full h-16 bg-primary/5 border border-primary/10 rounded-[1.75rem] flex items-center px-6 gap-4 active:scale-[0.98] transition-all outline-none select-none ring-0"
                >
              ...
              {(userProfile?.isAdmin || userProfile?.isCoinseller) && (
                <button 
                  ...
                  className="w-full h-16 bg-amber-500/5 border border-amber-500/10 rounded-[1.75rem] flex items-center px-6 gap-4 active:scale-[0.98] transition-all outline-none select-none ring-0"
                >
              ...
              {(userProfile?.isAdmin || userProfile?.isSupport) && (
                <button 
                  ...
                  className="w-full h-16 bg-zinc-500/5 border border-zinc-500/10 rounded-[1.75rem] flex items-center px-6 gap-4 active:scale-[0.98] transition-all outline-none select-none ring-0"
                >
                  <div className="w-10 h-10 rounded-xl bg-zinc-500 text-white flex items-center justify-center relative">
                    <ShieldAlert className="w-5 h-5" />
                    {pendingReportsCount > 0 && (
                      <span className="absolute -top-1 -right-1 w-4 h-4 bg-primary rounded-full border-2 border-white flex items-center justify-center text-[8px] font-black text-white">
                        {pendingReportsCount}
                      </span>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <button 
            ...
            className="w-full h-20 glass-card rounded-[2.25rem] flex items-center px-6 gap-5 native-shadow group active:scale-[0.98] transition-all outline-none select-none ring-0"
          >
            <div className="w-12 h-12 rounded-[1.25rem] bg-gradient-to-tr from-primary to-orange-600 flex items-center justify-center shadow-lg shadow-primary/20 transition-transform group-hover:scale-105">
          ...
          {userProfile?.isAgent && (
            <button 
              ...
              className="w-full h-20 bg-zinc-950 rounded-[2.25rem] flex items-center px-6 gap-5 active:scale-[0.98] transition-all native-shadow outline-none select-none ring-0"
            >
              <div className="w-12 h-12 rounded-[1.25rem] bg-zinc-800 flex items-center justify-center shadow-lg border border-white/5">
          ...
          {userProfile?.gender?.toLowerCase() === 'female' && !userProfile?.isAgent && (
            <button 
              ...
              className="w-full h-20 glass-card rounded-[2.25rem] flex items-center px-6 gap-5 native-shadow group active:scale-[0.98] transition-all outline-none select-none ring-0"
            >
              <div className="w-12 h-12 rounded-[1.25rem] bg-primary/10 flex items-center justify-center">
                <Building2 className="w-6 h-6 text-primary" />
              </div>
              <div className="flex-1 text-left">
        ...
        <div className="space-y-4">
          ...
          <button 
            ...
            className="w-full h-16 glass-card rounded-[1.75rem] flex items-center justify-between px-6 native-shadow transition-all active:scale-[0.98] outline-none select-none ring-0"
          >
            <div className="flex items-center gap-4">
              <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", isVerified ? "bg-primary/10 text-primary" : "bg-gray-50 text-gray-400")}>
            ...
            {isVerified ? <CheckCircle className="w-5 h-5 text-primary" /> : <ChevronRight className="w-4 h-4 text-gray-300" />}
          </button>

          <button 
            onClick={handleContactSupport} 
            className="w-full h-16 glass-card rounded-[1.75rem] flex items-center px-6 gap-4 native-shadow transition-all active:scale-[0.98] outline-none select-none ring-0"
          >
